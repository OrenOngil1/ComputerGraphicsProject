...
{
    // Read into locals; db/waypoints are only touched once everything checks
    ...
    if (!hasShape(anchorMat, kAnchorCols, CV_32F) || descriptors.type() != CV_8U ||
        descriptors.rows != anchorMat.rows) {
        std::cerr << "FEATURES: " << path << " is malformed -- descriptor rows and"
                  << " anchors must pair up one to one" << std::endl;
        return false;
    }
