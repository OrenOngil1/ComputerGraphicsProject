...
// A named lighting setup, for the L key to cycle through (see keyCallback).
struct LightPreset {
    const char      *name;   // printed to the console when selected
    DirectionalLight light;
};

// Deliberately few and visually far apart: the runtime control exists so the
// scene's appearance can *change markedly* (Mode 4 measures how feature
// matching survives a lighting change), not for fine-tuning. [0] is the
// startup default Simulation initializes from.
