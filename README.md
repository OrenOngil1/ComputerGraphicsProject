...
- **Mode D (SIFT + RANSAC), manual anchoring.** Pre-phase is by hand: for each
  ...
  **RANSAC PnP** for robustness to wrong matches. (SIFT rather than ORB by
  ...
  generates a test database automatically — a high survey circle covering the
  whole map, human aim error simulated as depth noise along each sight line —
  for experiments; the hand build remains the mode. `Ctrl+S`/`Ctrl+O` keep the hand-built
